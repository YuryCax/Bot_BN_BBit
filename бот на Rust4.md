...
**Архитектура:** Trading bot — Rust (`Observer` thin forwarder → `Executor` entry+risk); **Analyst** — offline-советник (§8.6, Фаза 2); **Фаза 3** — validated Analyst → интеграция или 2-й акк (§10.7). Старт: **t3.micro Tokyo + t3.small Singapore** (§2.4). **Ресурсы:** §2.7. **Конфиги:** `config/` (§10.1). **ADR-003** supersedes ADR-001 (Singapore entry).
...
[ Server A: Observer ] — AWS Tokyo (ap-northeast-1)
   • simd-json parser (bookTicker)
   • Zenoh publish BinanceTick (UDP)
...
[ Server B: Executor ] — AWS Singapore (ap-southeast-1)
   ...
   • Local Bybit mid WS
   • Entry Engine (§7) + LagState + follow-through
   • Risk Engine (adverse_move ≠ book_slippage) + warm flags
   • Position Manager (Lag Convergence + Time Stop)
          ↓
[ Bybit API ] → USDT Perpetual
```

**Ключевые принципы (v2.4 / ADR-003):**
...
3. **Spot и Futures разделены** на уровне коннекторов и депозитов (§6.0).
4. **Long и Short** — симметричная логика (§7).
...
**Учёт расходов за год (старт t3.micro + t3.small):**

| Статья | ~/год |
|--------|-------|
| AWS t3.micro Tokyo + t3.small Singapore | $80–200 |
...
- **Транспорт:** UDP (порт 7447), без гарантии доставки; **обязателен `seq_num`** в каждом `BinanceTick` для детекции потерь и дедупликации.
...
- **Топики (ADR-003):**
  ...
  - Local `MarketStatePacket` log на Executor после Entry §7 (replay)
...
**Общие sysctl (оба узла — старт и scale):**
```bash
...
**Старт (`t3.micro` / `t3.small`) — без `taskset`:**
```bash
# Observer (Tokyo, t3.micro): 2 tokio workers — §2.7.1, §3.1
./observer

# Executor + Panel (Singapore, t3.small): executor + control-panel + telegram sidecar
./executor
./control-panel
```

**[scale-only] Pinning ядер — только `c7a.xlarge` и выше (§2.5 Scale C):**
```bash
...
|------|--------|----------|---------|------------|
| Observer | `ap-northeast-1` Tokyo | **t3.micro** | `observer.service` | ≤ 400 MiB |
| Executor + Panel | `ap-southeast-1` Singapore | **t3.small** | `executor.service`, `control-panel.service`, `telegram-alerts.service` | ≤ 650 MiB суммарно |
...
| Этап | Условие | Инфра |
|------|---------|-------|
| **Старт** | Paper + live 1% | t3.micro Tokyo + **t3.small** Singapore |
| **Scale A** | RAM/CPU throttle | SG → t3.medium или Tokyo → t3.small |
...
Один сервер Singapore in-process — опционально для **локальной отладки** перед dual-node deploy:

| Параметр | Mono-node | Dual-node (paper/live) |
|----------|-----------|------------------------|
...
#### 2.7.1. Бюджет железа (старт: t3.micro Tokyo + t3.small Singapore)
...
| **Singapore** | `telegram-alerts` | **≤ 50 MiB** | sidecar; **t3.small** обязателен для paper/live (§2.4) |
...
- Observer публикует `system/heartbeat` ~10 Hz.
...
| Paper Trading | Bybit Testnet + paper ledger | ≥ 100 сделок Futures; DD < 10%; net after fees |
| Live Staged | **1% депозита**, ×3 | 7 дней без критических ошибок |
...
> **Канонические файлы репозитория:** `config/config.toml`, `config/symbols.toml`, `config/edge_profile.toml` (после §9.0). На сервере: `/etc/bot/`. Секреты — env / Parameter Store (§8.4).
...
executor_instance_type = "t3.small"    # paper/live: не micro (§2.4)
spot_enabled = false
min_futures_pf_for_spot = 1.3
start_futures_pairs = ["BTCUSDT", "ETHUSDT"]   # + опц. SOLUSDT
max_symbols = 35                       # SymbolRegistry §3.4
edge_profile_path = "config/edge_profile.toml"  # output §9.0; обязателен для live

[resources]                            # §2.7 — budget
...
tokio_worker_threads = 2               # observer/executor на micro/small

[lag]                                  # §3.5, §7 — fine-tune из edge_profile после §9.0
impulse_min_bps = 5.0
lag_min_bps = 3.0                      # placeholder до §9.0
follow_through_min = 0.40              # override per-symbol в edge_profile.toml
convergence_exit_ratio = 0.75          # exit: Bybit догнал 75% импульса
time_stop_ms = 8000
capture_est = 0.6                      # net_edge est. в Entry Engine
bybit_mid_feed_hz = 50                 # Executor → Observer reverse feed
bybit_mid_max_staleness_ms = 200       # stale → entry_valid=0 (fail-closed)
bybit_mid_feed_timeout_ms = 500        # потеря feed → halt entries + alert
...
packet_version = 3                     # = PACKET_VERSION §10.4; mismatch → refuse start
...
### 10.2. `symbols.toml` (MVP: futures-only; spot — фаза 2)

> **MVP (`spot_enabled = false`):** все enabled-пары — `instrument = "futures"`. Spot-примеры — только в комментариях.

```toml
# MVP Фаза 1 — futures-only, $300 депозит, ×10 (§2.4)
# Сумма futures_alloc_pct по enabled ≤ 1.0
...
instrument = "futures"
leverage = 10
enabled = false              # включить после §9.0 pass на SOL
futures_alloc_pct = 0.10

# --- Spot (фаза 2, после min_futures_pf_for_spot) ---
# [[symbol]]
# id = 4
# binance = "ETHUSDT"
# bybit = "ETHUSDT"
...
═══════════════════════════════════════════════════════════════════
  Этап 1.1  Dual-node: t3.micro Tokyo + t3.small Singapore (§2.4) или mono-node отладка (§2.6)
...
## 23. Изменения v2.2 → v2.3

| # | Проблема v2.2 | Исправление v2.3 |
|---|---------------|------------------|
| 1 | `packet_version` 2 в config vs 3 в §2.2/§10.4 | Единое значение **3**; mismatch → refuse start |
| 2 | `symbols.toml`: ETH spot при `spot_enabled = false` | MVP **futures-only**; spot в комментариях |
| 3 | Lag: «Observer **или** Executor warm merge» | §3.5.1 **Observer-only** lag; Executor публикует feed 50 Hz |
| 4 | Нет staleness/fallback для `bybit_mid` | `bybit_mid_max_staleness_ms`, feed timeout, fail-closed |
| 5 | §2.3 `taskset` без пометки scale | `[scale-only]` + отдельный блок для micro/small |
